...
func renderIntakeBlock(fields []IntakeField, facts []Fact, prevFacts []PreviousFact) string {
	...
	b.WriteString("    редактор их замаскировал для приватности. Считай контакт полученным:\n")
	b.WriteString("      • НЕ проси клиента повторить телефон/почту, если уже видел маску;\n")
